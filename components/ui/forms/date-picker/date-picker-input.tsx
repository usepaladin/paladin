    ...
    (options: DateTimeInputProps, ref) => {
        ...
        const inputStr = useMemo(() => {
            return segments
                .map((s) =>
                    s.value
                        ? s.value.padStart(s.symbols.length, "0")
                        : s.symbols
                )
                .join("");
        }, [segments]);
        ...
        const onClick = useEventCallback(
            (event: React.MouseEvent<HTMLInputElement>) => {
                ...
                if (
                    ...
                    !segment &&
                        (segment = validSegments.find(
                            (s) => s.index >= selectionStart
                        ));
        ...
        const onSegmentNumberValueChange = useEventCallback(
            (num: string) => {
                ...
                if (segment.type !== "period") {
                    ...
                    let newValue =
                        rawValue.length < length ? rawValue + num : num;
                    ...
                    if (!shouldNext) {
                        switch (segment.type) {
                            ...
                            case "hour":
                                shouldNext =
                                    +newValue >
                                    (segment.symbols.includes("H") ? 2 : 1);
                                break;
                            case "minute":
                            case "second":
